...
merge — and every model call is routed so you can dial cost and capability per
role.**
...
- **budgets enforced somewhere your bugs can't reach**.
...
- **Steering from anywhere.** Anything that can flip a label is a control
  surface: your terminal, the GitHub app on your phone, a Telegram button.
...
│   memos · worktrees · budgets · Telegram thread                │
└──────────────┬─────────────────────────────────────────────────┘
               │ one stage handler per state
┌──────────────▼─────────────────────────────────────────────────┐
│ ROLE AGENTS — the workers                                      │
│   design agent · design reviewers ×2 ·                         │
│   coder (Claude Code, headless) · code reviewers ×2            │
│   each = a system prompt + a model + skills; stateless         │
└──────────────┬─────────────────────────────────────────────────┘
               │ metered, budget-capped calls
┌──────────────▼─────────────────────────────────────────────────┐
│ MODEL PROVIDERS                                                │
│   OpenRouter (every family behind one API, per-role caps)      │
│   Anthropic (the coder's harness: claude -p in the worktree)   │
└────────────────────────────────────────────────────────────────┘
```
...
loop. (Deliverator keeps a harness-free fallback, `CODER_MODE=oneshot`, mostly
as a demonstration of exactly how much a harness buys.)
...
| Budget caps | one spend-capped API key per role, enforced on the provider's servers | a runaway loop burning your account |
...
Reviewer trust gets one extra trick: **diversity beats collusion.** If the
reviewer and the coder are the same model — or two reviewers share a family —
they share blind spots and tend to bless each other's mistakes. Deliverator's
reviewers come from *different model families* than the coder (DeepSeek and
Qwen against an Anthropic coder, by default), and they see the artifact with
fresh context, never the coder's reasoning. They judge what was built, not
what was intended.

## Models and money

Each role gets the cheapest model that's good enough, and you upgrade only
roles that visibly fail. The coder is the exception — it does the hardest
work, so it starts mid-tier. Routing through OpenRouter makes any swap a
one-line change in `roles.py`.

Money is controlled in four independent layers, from hard to soft:

```text
  layer 1  account balance      the ultimate ceiling — keep it low while learning
  layer 2  per-role capped keys enforced on OpenRouter's servers: when the
                                coder key hits $10/day, the "no" comes from the
                                provider — no bug of yours can spend past it
  layer 3  MAX_USD_PER_RUN      the loop stops itself mid-pass
  layer 4  the cost dashboard   visibility: spend by role and by day, from the
                                CSV every call appends to
```

A realistic solo loop lands in low single-digit dollars per day. The caps
exist for the bad day — the runaway retry — not the normal one. The headless
coder's spend doesn't flow through OpenRouter (it's metered on your Anthropic
key), but its per-session cost is logged into the same books, its sessions are
bounded by `--max-turns`, and the in-loop cap sees it. One set of books, two
providers.
...
one line in `roles.py`. In that order of cost. The loop improves by editing
