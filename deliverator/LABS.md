...
Let the loop run a pass (watch the log, or wait for the Telegram ping). It
...
5. Park at `agent:needs-human` — your Telegram pings. Your turn.
...
> cross-family reviewers, and touch point 3 exist. Resisting the urge to jump
...
an upgrade to the rubber-stamping reviewer's model — one line in `roles.py`.
...
- **Verification is layered, and nothing certifies its own work.** The coder's
  inner loop, the independent Docker check, CI, two cross-family reviewers,
  your hands.
- **Correct the system, not the code.** A rubber-stamped missing test means: a
  comment (it feeds the memo), a line in AGENTS.md, or a model upgrade in
  `roles.py`. The loop improves by editing its parts, not by you doing its job.
